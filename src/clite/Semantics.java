...
public class Semantics {
    ...
    State M (Statement s, State state) {
        ...
        if (s instanceof AntiConditional) return M((AntiConditional)s, state); 
    ...
    State M (AntiConditional c, State state) {
    	if (M(c.test, state).boolValue()) {
    		return M (c.elsebranch, state);  
    	}
    ...
    Value applyBinary (Operator op, Value v1, Value v2) {
        ...
        if (op.val.equals(Operator.INT_DIV)) 
            return new IntValue(v1.intValue( ) / v2.intValue( ));
        if (op.val.equals(Operator.INT_SUPERPLUS))
        	return new IntValue(v1.intValue( ) + v2.intValue( ) + 1); 
        // student exercise
	...
	if (op.val.equals(Operator.FLOAT_PLUS)) 
        ...
        if (op.val.equals(Operator.FLOAT_DIV)) 
            return new FloatValue(v1.floatValue( ) / v2.floatValue( ));
    	if (op.val.equals(Operator.FLOAT_SUPERPLUS))
    		return new FloatValue(v1.floatValue( ) + v2.floatValue( ) + 1); 
        
    if (op.val.equals(Operator.SEX_EQ))
    		return new BoolValue(v1.sexValue( ) == v2.sexValue( )); 
    	if (op.val.equals(Operator.SEX_NE))
    		return new BoolValue(v1.sexValue( ) != v2.sexValue( )); 
