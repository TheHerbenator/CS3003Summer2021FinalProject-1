...
public class Parser {
	...
	if (token.type().equals(TokenType.Int)) {
		...
		t = Type.FLOAT;
	} else if (token.type().equals(TokenType.Sex)) {
		t = Type.SEX; 
	} else if (token.type().equals(TokenType.Void)) {
		t = Type.VOID;
	} else error("int | bool | float | char | sex");
        // student exercise
        return t;          
    }
	...
	if (token.type().equals(TokenType.LeftBrace)) {
		...
		s = ifStatement();
	} else if (token.type().equals(TokenType.IfNot)) {
		s = ifNotStatement(); 
    ...
    private AntiConditional ifNotStatement() {
    	match(token.type()); 
    	match(TokenType.LeftParen); 
    	Expression test = expression(); 
    	match(TokenType.RightParen); 
    	Statement tp = statement();
        return new AntiConditional(test, tp);  // student exercise
    } 
    ...
    private boolean isAddOp( ) {
        return token.type().equals(TokenType.Plus) ||
               ...
               token.type().equals(TokenType.SuperPlus);
    }
    ...
    private boolean isType( ) {
        return token.type().equals(TokenType.Int)
            ...
            || token.type().equals(TokenType.Sex)
    ...
    private boolean isSexLiteral( ) {
    	return token.type().equals(TokenType.Male) ||
    		token.type().equals(TokenType.Female);
    }
