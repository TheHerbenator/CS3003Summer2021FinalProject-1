...
class Program {
    ...
    String inner_display(String spc, String spcing, Object node) {
		...
		} if (node instanceof AntiConditional) {
			AntiConditional ancon_node = (AntiConditional) node; 
			String prefix = spcing + "AntiConditional:\n"; 
			String test = inner_display(spc, spc + spcing, ancon_node.test); 
			String then_branch = spcing + "then:\n" + inner_display(spc, spc + spcing, ancon_node.thenbranch);
			return prefix + test + then_branch;  
...
class Type {
    // Type = int | bool | char | float | void | sex
    final static Type INT = new Type("int");
    final static Type BOOL = new Type("bool");
    final static Type CHAR = new Type("char");
    final static Type FLOAT = new Type("float");
    final static Type VOID = new Type("void");
    final static Type SEX = new Type("sex"); 
	...
	if(this.equals(Type.INT) || this.equals(Type.BOOL) || this.equals(Type.CHAR) || this.equals(Type.SEX))
		return "I";
...
class AntiConditional extends Statement {
// definition like in Appendix goes here
	Expression test; 
	Statement thenbranch, elsebranch; 
	
//	AntiConditional (Expression t, Statement tp) {
//		test = t; thenbranch = tp; 
//	}
...
abstract class Value extends Expression {
    ...
    static Value mkValue (Type type) {
        ...
        if (type == Type.SEX)	return new SexValue( );  
...
class SexValue extends Value { 
	private String value = ""; 
	
	SexValue ( ) { type = Type.SEX; }
	
	SexValue (String v) { this( ); value = v; undef = false; }
	
	String sexValue ( ) {
		assert !undef : "reference to undefined sex value"; 
		return value; 
	}
...
class Operator {
    ...
    final static String INT_SUPERPLUS = "INT+*";
    // UnaryOp = !    
    ...
    final static String FLOAT_SUPERPLUS = "FLOAT+*"; 
    // UnaryOp = !    
    ...
    // UnaryOp = ! 
    final static String SEX_NEG = "SEX-NEG"; 
    // RelationalOp = == | !=
    final static String SEX_EQ = "SEX=="; 
    final static String SEX_NE = "SEX!="; 
    // Type specific cast
    ...
    boolean ArithmeticOp ( ) {
        return val.equals(PLUS) || val.equals(MINUS)
            ...
            || val.equals(SUPERPLUS) || val.equals(INT_SUPERPLUS) || val.equals(FLOAT_SUPERPLUS);
    }
    ...
    boolean sexOp ( ) { return val.equals(SEX); }
    ...
    final static String sexMap[ ] [ ] = {
    	{EQ, SEX_EQ}, {NE, SEX_NE}, {NOT, SEX_NEG}
    };
    ...
    final static public Operator sexMap (String op) {
    	return map (sexMap, op); 
    }
}
